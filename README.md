...
> New to this repo? Start with **`HANDOFF.md`**, which summarises what the site contains and
> which parts are intentionally the way they are.

Plain static HTML/CSS/JS site for Home State Inspections (Boaz Silverberg), replacing the near-empty homestateinspects.com placeholder. No framework, no build step, matching the same locked decision used on the Warp9 Drones rebuild: zero moving parts, native fit for free GitHub Pages hosting.
...
Most of `images/*.jpg` are stock photos (one per service on `services.html`) sourced from Unsplash, free for commercial use under the [Unsplash License](https://unsplash.com/license). No attribution is required, though it's good practice. These are placeholders: swap in real photos of Boaz's own inspections/equipment whenever available, which read as more authentic than stock photography.

`thermal-imaging.jpg` is the exception: it is a real photo Boaz took himself of an actual thermal-imaged breaker panel (with genuine temperature readouts), so no licensing question at all. This is the model for the rest: as real inspection photos come in, they should replace the stock placeholders one by one.
...
`logo-source.jpeg` is the original file (1535x1024, plain white background). `logo-transparent.png` is the usable asset generated from it, with the background removed and auto-cropped to content (1308x832), used across the site at 52px tall in the page header and 108px in the homepage hero.
